...
function getPolynom(...rest) {
  return function createPolinom(x) {
    const args = [...rest].reverse();
    let k = -1;
    return args.reduce((accom, arg) => {
      k += 1;
      return accom + arg * x ** k;
    }, 0);
  };
}
...
function logger(func, logFunc) {
  return function cosLogger(...args) {
    ...
    const msgStart = `${func.name}(${argsStr}) starts`;
    const msgEnd = `${func.name}(${argsStr}) ends`;
